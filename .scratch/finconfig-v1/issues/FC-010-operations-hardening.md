...
- Replaced the ambiguous process `Environment` configuration with `RuntimeMode` and added a strict Config Server profile requiring a case-sensitive ManagedEnvironment, non-zero ServerEpoch UUID, MySQL and a private UDS; environment variables override the two identities independently and SafeSummary redacts the DSN.
- Enforced the ManagedEnvironment before SDK snapshot reads, DiffVersions/GetCollections fallbacks, Watch subscription, QueryPage, Diagnostics and RefreshHint enqueue. Cross-environment reads map to FailedPrecondition, cross-environment hints to InvalidArgument, missing snapshots to Unavailable and missing PageQuery models/collections to NotFound.
- Verified this slice with focused package tests, `go test ./...`, and `go test -race ./internal/platform/config ./internal/configserver/... ./internal/distribution/snapshot ./internal/pagequery/...`; follow-up Standards and Spec reviews reported no open hard finding.
