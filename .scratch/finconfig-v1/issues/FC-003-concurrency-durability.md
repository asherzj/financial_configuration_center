...
- Completed the public local SDK surface for primary-key `QueryOne`/`QueryMany`, stable `QueryAll`, version inspection, immutable rows, explicitly tagged scalar `Decode`/`DecodeMany`, and bounded asynchronous collection subscriptions. Callback panics are isolated and saturated notifications never affect the already-published snapshot.
