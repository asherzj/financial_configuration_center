...
func TestGetSnapshotEnforcesSubscriptionAndEnvironment(t *testing.T) {
	...
	response, err := service.GetSnapshot(context.Background(), configserver.GetSnapshotRequest{ConsumerID: "other", ClientID: "pod", Environment: "production", KnownVersions: []configserver.Version{{Collection: "old", Revision: 1, Digest: "digest"}}})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := service.GetSnapshot(context.Background(), configserver.GetSnapshotRequest{ConsumerID: "other", ClientID: "pod", Environment: "staging"}); err == nil {
