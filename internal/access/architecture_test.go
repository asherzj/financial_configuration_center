...
func TestAccessDoesNotImportServerImplementations(t *testing.T) {
	t.Parallel()
	...
	err := filepath.WalkDir(filepath.Join(root, "internal/access"), func(path string, entry os.DirEntry, walkErr error) error {
		if walkErr != nil {
			return walkErr
		}
