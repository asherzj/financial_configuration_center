...
func ComputeDigest(rules []Rule) (readmodel.Digest, error) {
	...
	for index, rule := range ordered {
		if index > 0 && ruleIdentity(ordered[index-1]) == ruleIdentity(rule) {
			return readmodel.Digest{}, fmt.Errorf("%w: duplicate rule for %s", ErrInvariant, ruleIdentity(rule))
		}
		ranges, err := canonicalRanges(rule.RolloutRanges)
		if err != nil {
			return readmodel.Digest{}, fmt.Errorf("compute overlay digest: rule %q: %w", rule.ID, err)
		}
		payload[index] = []any{
			rule.Collection,
			rule.Scope.Region,
	...
	if err != nil {
		return readmodel.Digest{}, fmt.Errorf("compute overlay digest: %w", err)
	}
	sum := sha256.Sum256(encoded)
	return readmodel.Digest{Algorithm: "SHA-256", Value: hex.EncodeToString(sum[:])}, nil
}
