...
type Rule struct {
	...
	ConfigRevision    readmodel.ConfigRevision  `json:"configRevision"`
	ReleaseOrderID    string                    `json:"releaseOrderId"`
	EffectiveFrom     *time.Time                `json:"effectiveFrom,omitempty"`
	EffectiveUntil    *time.Time                `json:"effectiveUntil,omitempty"`
	ActivatedRevision *readmodel.ConfigRevision `json:"activatedRevision,omitempty"`
	ActivatedAt       *time.Time                `json:"activatedAt,omitempty"`
	ExpiredRevision   *readmodel.ConfigRevision `json:"expiredRevision,omitempty"`
...
func Evaluate(query Query, base []readmodel.ConfigurationRecord, rules []Rule) ([]readmodel.ConfigurationRecord, error) {
	records := make(map[string]readmodel.ConfigurationRecord, len(base))
	...
	for _, stage := range []string{"", query.Scope.Stage} {
		for _, rule := range rules {
			...
			case ActionAdd:
				...
				record = readmodel.ConfigurationRecord{
	...
	sort.Strings(keys)
	effective := make([]readmodel.ConfigurationRecord, len(keys))
